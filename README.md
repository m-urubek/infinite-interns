...
```mermaid
flowchart TD
    START(["start"]) --> analysisCtr["Analysis Controller"]

    analysisCtr --> prdGen["PRD Generator"]
    prdGen --> analysisCtr

    analysisCtr -->|Business phase & enabled| bizAnalyze["Business PRD Analyzer"]
    analysisCtr -->|Technical phase & enabled| techAnalyze["Technical PRD Analyzer"]
    analysisCtr -->|Analysis complete| initDoc["Initial Documenter"]
    ...
    analysisCtr -->|Needs clarification| bizClarify["Business Clarification Answerer"]
    analysisCtr -->|Needs clarification| techClarify["Technical Clarification Answerer"]
...
- **Technical PRD Analyzer**: Analyzes the PRD against technical requirements and architecture to identify gaps and generates clarifying questions for the user.
- **Business Clarification Answerer**: Automatically answers business requirement clarification questions generated by the Business PRD Analyzer (user-configurable).
- **Technical Clarification Answerer**: Automatically answers technical clarification questions generated by the Technical PRD Analyzer (user-configurable).
...
- **Microplanner**: Creates a micro-plan for the implementer. It analyzes the current codebase patterns and determines which ones to use. This is an attempt to limit the situations when an implementing agent overlooks certain solutions the codebase already has and tries to re-invent them.
- **Initial Documenter**: After the prd is created, it creates new documentation files or modifies existing ones with a tag "currently implementing" (in the new files and also in the modified blocks in existing documentation).
- **Micro Documenter**: After each task is finished and verified, the micro documenter determines whether documentation needs updating with new information that had to be unexpectedly researched or added as part of the task and if it does, it modifies it in the same way as the Initial Documenter.
- **Final Documenter**: Removes the "currently implementing" tags and polishes the overall documentation added.
- **Tests Generator**: After each task, it determines whether the new code deserves a separate unit test or enhancement to existing unit tests and if so, it implements it. This can also modify the build command to run tests if not part of it yet.
- **Documentation Indexer**: Maintains a master documentation index file that serves as the entry point to all documentation. The index provides a structured map to other documentation files and code blocks, allowing all LLM agents to efficiently query only relevant documentation without needing to read entire documentation files (which can become huge). This index is always fed to agents along with their prompts to enable context-aware documentation lookups.

**Nodes** (Deterministic logic):

- **Analysis Controller**: Routes between business and technical analysis phases, handles clarification round limits, and decides when to move to documentation.
...
- **Business Clarifications**: Toggle Business PRD Analyzer and Business Clarification Answerer (disabled/interactive/auto modes)
- **Technical Clarifications**: Toggle Technical PRD Analyzer and Technical Clarification Answerer (disabled/interactive/auto modes)
...
### Analysis Phase Settings (Implemented)
...
- **Clarification Modes**: disabled/interactive/auto per analysis type — controls whether questions are skipped, presented to the human, or auto-answered by an agent

### Agent Settings (Implemented)

Each LLM agent can be configured individually:

- **Model**: LLM model selection per agent (29+ Gemini models supported, manual entry for OpenAI/DeepSeek)
- **Temperature**: Temperature setting (0-2) per agent
- **Thinking**: Enable/disable reasoning output per agent (Google/Gemini only)
...
- **Custom Rules**: Custom instructions per agent (plain text, appended to agent system prompt)
- **Provider**: LLM provider selection (Google/OpenAI/DeepSeek) — global per preset, affects all agents
...
Global rate limit configuration per preset with enforcement:

- **Max RPM** (Requests Per Minute): Rate limit for API requests (enforced via sliding window)
- **Max TPM** (Tokens Per Minute): Token consumption rate limit (soft cap — stored but not enforced pre-request)
- **Max RPD** (Requests Per Day): Daily request limit (enforced via sliding window)
- **Max Spending**: Maximum daily spending in USD (soft cap — stored but not enforced pre-request)

### Documentation Settings (Implemented)

Global documentation control:
...
- **Agents** (LLM-powered): PRD Generator, Business/Technical Analyzers, Clarification Answerers, Planner, Microplanner, Implementer, Verifier, Tests Generator, Final Verifier, Initial/Micro/Final Documenters, Documentation Indexer
- **Nodes** (Deterministic): Analysis Controller, Implementation Controller, Builder, Answer Clarifications
- **Backends**: Read-only, Read+Execute, and Full-access permission modes
- **Frontend**: React dashboard with phase tracking, interrupt handling, and preset management

## Technologies

- LangChain + LangGraph
- Google Gemini (gemini-3-flash-preview), OpenAI, DeepSeek (multi-provider support)
